...
namespace Dissonance.Networking.Server
{
        ...
        : BaseClientCollection<TPeer>
    {
        ...
        public void ProcessHandshakeRequest(TPeer source, ref PacketReader reader)
        {
            ...
            // We'll assume name collisions never happen, so a is probably a client reconnecting before the server has cleaned up after a very recent disconnection
            ...
            var writer = new PacketWriter(_tmpSendBuffer);
            writer.WriteHandshakeResponse(_server.SessionId, info.PlayerId, PlayerIds, ClientsInRooms.ByName);
